#!/usr/bin/env bash
# Bascule Sonos Scheduler entre mode protégé (oauth2-proxy/Authentik) et ouvert.
#
...
APP="$UNIT_DIR/sonos-scheduler.container"
PROXY="$UNIT_DIR/sonos-scheduler-auth.container"
ENVFILE="$HOME/.config/sonos-scheduler/oauth2-proxy.env"
...
set_bind() {  # <host> <port> : (ré)écrit les Environment=BIND_* dans le .container de l'app
  ...
  sed -i "\#^Image=localhost/sonos-scheduler#a Environment=BIND_HOST=$1\nEnvironment=BIND_PORT=$2" "$APP"
}
...
case "$MODE" in
  auth)
    ...
    cp "$HERE/sonos-scheduler-auth.container" "$PROXY"
    systemctl --user daemon-reload
    systemctl --user restart sonos-scheduler.service
    systemctl --user start sonos-scheduler-auth.service
    echo "✅ Mode AUTH : oauth2-proxy :8095  ->  app interne 127.0.0.1:8096"
    ;;
  noauth)
    systemctl --user stop sonos-scheduler-auth.service 2>/dev/null || true
    rm -f "$PROXY"
